...
import { useEditorDetail } from './hooks/use-editor-detail'
import type { EditorJob, EditorJobStatus } from './types/editor-performance.types'
...
const STATUS_STYLES: Record<EditorJobStatus, { bg: string; text: string; label: string }> = {
...
function StatusBadge({ status }: { status: EditorJobStatus }) {
...
function PaymentCell({ job }: { job: EditorJob }) {
  if (job.status === 'COMPLETED' && job.editor_payment !== null)
    return <span className="font-medium text-gray-800 text-sm">{formatRupiah(job.editor_payment)}</span>
...
export function EditorDetailPage() {
  ...
  const { data, isLoading, isError, isFetching } = useEditorDetail(
    editorId,
    pagination.pageIndex + 1,
    pagination.pageSize,
  )

  const editor = data?.profile ?? null
  const stats  = data?.stats   ?? null
  const jobs   = data?.jobs    ?? []

  const columns = useMemo<MRT_ColumnDef<EditorJob>[]>(() => [
    {
    ...
    },
    {
      ...
      header: 'Status',
      size: 140,
      Cell: ({ cell }) => <StatusBadge status={cell.getValue<EditorJobStatus>()} />,
    },
    {
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <button
          onClick={() => navigate(`/admin/editors/${editorId}/jobs/${row.original.id}`)}
  ...
  const table = useMantineReactTable({
    ...
    mantineToolbarAlertBannerProps: isError
      ? { color: 'red', children: <span role="alert">Failed to load job history.</span> }
      : undefined,
  ...
  if (!editor || !stats) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3">
        <p className="text-text-secondary">Editor not found.</p>
        <Link to="/admin/editors" className="text-sm text-primary hover:underline">← Back to Editors</Link>
      </div>
    )
  }
      ...
      <div className="bg-white rounded-2xl border border-border-base overflow-hidden">
        <div className="bg-primary-subtle px-6 py-5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-text-primary">{editor.name}</h1>
          </div>
          <AvailabilityBadge active={editor.has_active_job} />
        </div>
        <div className="px-6 py-4 border-t border-border-base flex flex-col gap-1.5">
          ...
          <span className="flex items-center gap-2 text-sm text-text-tertiary">
            ...
            Member since {formatDate(editor.created_at)}
          </span>
        </div>
      </div>
